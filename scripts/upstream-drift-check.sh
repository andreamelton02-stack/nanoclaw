#!/bin/bash
# Weekly upstream-drift check for the Jeeves fork.
#
# Compares andreamelton02-stack/nanoclaw vs qwibitai/nanoclaw upstream/main.
...
FORK="andreamelton02-stack/nanoclaw"
...
# 9. Real run: create the issue using Andrea's token (scoped to this command only)
url=$(GH_TOKEN="$ANDREA_TOKEN" gh issue create --repo "$FORK" --title "$title" --body "$body" --label drift-report)
echo "ISSUE: $url"
